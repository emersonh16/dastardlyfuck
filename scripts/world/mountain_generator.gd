# MountainGenerator - Utility class for generating mountain clusters
# Similar to rock cluster generation from JavaScript reference

class_name MountainGenerator

const CELL_SIZE: float = 2.0  # Object cell size (matches miasma tile size)
const BASE_TILE_SIZE: float = 64.0  # Ground tile size

# Generate mountain cluster mask (similar to rock.buildClusterMask)
# Returns Dictionary with cells array and tall Set
static func build_mountain_cluster(seed_value: int, wind_dir: float = 0.0) -> Dictionary:
	...
	# Mountain parameters (larger than rocks)
	var base_steps = 250
	var steps = int(base_steps * jitter.call(1.0, 1.5))
	var branch_steps = 80
	var branch_prob = 0.12
	var walkers = 4 + (1 if rng.randf() < 0.5 else 0)  # 4-5 walkers
	...
	# Carve wind-facing direction (mountains eroded by wind)
	_carve_wind_direction(cell_set, dirs4, dirs8, wind_dir, rng)
	
	# Mark tall cells (these block miasma)
	var tall = _mark_tall_cells(cell_set, dirs4, dirs8, rng)
...
static func _mark_tall_cells(cell_set: Dictionary, dirs4: Array, _dirs8: Array, rng: RandomNumberGenerator) -> Dictionary:
	var tall = {}
	var tall_seeds = 8 + int(rng.randi() % 6)
	var cells_arr = []
	for k in cell_set:
		cells_arr.append(k)
	...
	for s in range(tall_seeds):
		...
		var budget = 25 + int(rng.randi() % 15)
...
# Check if a position is blocked by tall mountain cells
static func is_position_blocked(mountain_data: Dictionary, world_pos: Vector3, mountain_world_pos: Vector3) -> bool:
	if not mountain_data.has("tall"):
		return false
	
	var tall_cells = mountain_data.tall
	var cell_size = CELL_SIZE
	
	# Convert world position to relative position from mountain center
	var rel_x = world_pos.x - mountain_world_pos.x
	var rel_y = world_pos.z - mountain_world_pos.z
